...
EPILOG = dedent(
    ...
    Example call:
    ::
        {filename} -i /path/to/input_data_folder --config-file nnUNet_3D_config.json --task-ID 000
        {filename} -i /path/to/input_data_folder --config-file nnUNet_3D_config.json --task-ID 000 --test-split 25
    """.format(  # noqa: E501
...
def main():
    ...
    output_json_config_filename = "Dataset" + arguments["task_ID"] + "_" + config_dict["Experiment Name"] + ".json"
    ...
    output_file = Path(os.environ["ROOT_FOLDER"]).joinpath(
        config_dict["Experiment Name"], "Dataset_" + arguments["task_ID"] + "_" + TIMESTAMP + ".txt"
    )
